Create_object <- function(FilePath,
                          ...
                          color_gradientn = c('#313695','#6ca4cc','#cfebf3','#fee99d','#f67d4a','#b91426'),
...

{
  ...
  if(!exists('object')){
    ...
    object <- object %>%
      ...
      RunTSNE(reduction = "pca",dims = dims,check_duplicates = FALSE)
    }
    ...
    object <- FindNeighbors(object=object, reduction = "pca",dims = dims,verbose = F)
    object <- FindClusters(object=object, verbose = F,resolution = resolution)
    object <- RunUMAP(object=object, reduction = "pca",dims = dims)
    object <- RunTSNE(object=object, reduction = "pca",dims = dims,check_duplicates = FALSE)
  } 
  ...
  Space_heatmap <- function(Count_file, title = 'heatmap'){
    p <- ggplot(Count_file, aes(x = pos_w, y = (dim(png)[1] - pos_h)))+
  ...
  if (Cluster == TRUE) {
    ...
    write.csv(Cluster_df, file = paste(out_path,'Cluster.csv', sep = '/'), 
              quote = F, row.names = F)
    ...
    #p1
    cluster_pic <- ggplot(barcode_pos_cluster, aes(x = pos_w, y = (dim(png)[1] - pos_h)))+
      background_image(png)+
  ...
  if (Gene_stat == TRUE){
    ...
    gene_heatmap_func1 <- function(Gene_df, title_name) {
        ggplot(Gene_df, aes(x = pos_w, y = (dim(png)[1] - pos_h)))+
        #background_image(png)+
